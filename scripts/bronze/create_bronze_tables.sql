/*
==================================================================================
Bronze Layer : Create Table Script

This script prepares the Bronze layer tables for the E-Commerce Data Warehouse
using the Brazilian Olist dataset.

For each dataset table:
1. The script checks whether the table already exists in the bronze schema.
2. If the table exists, it is dropped to ensure a clean structure.
3. The table is then recreated with columns that match the original source
   CSV files.

These tables serve as the raw data ingestion layer where data is loaded
directly from source files before any transformations are applied in the
